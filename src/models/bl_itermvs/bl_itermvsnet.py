...
from .sub_nets import Pipeline
from src.utils.comm import (is_main_process, print0)
from src.utils.utils import print_indebugmode
...
class baseline_itermvs(nn.Module):
    def __init__(self, options):
        ...
        #if self.is_print_info_gpu0:
        #    print_indebugmode(f"depth_binning = {self.opt.depth_binning}")
        self.depth_binning = self.opt.depth_binning
        self.num_depth_bins = self.opt.num_depth_bins
        
        assert self.depth_binning == 'inverse', f"Baseline IterMVS requires inverse depth binning, but found {self.depth_binning}"
        assert self.num_depth_bins == 32, f"Baseline IterMVS uses M=32 plane hypotheses, but found {self.num_depth_bins}"
        
        self.depth = Pipeline(iteration=self.opt.itermvs_iters, test= self.is_test)
        ...
        pretrain_weights_path = self.opt.pretrain_itermvs_path
    ...
    def load_our_pretrained_ckpt(self, pretrain_weights_path):
        ...
        for k in ['depth', 'pose_encoder', 'pose']:
            try:
                getattr(self, k).load_state_dict(new_dict[k])
                if self.is_print_info_gpu0:
                    print(f"Loaded weights for model {k} from ckpt {pretrain_weights_path}")
...
class baseline_itermvs_eval(baseline_itermvs):
    ...
    def forward(self, inputs, frames_to_load, is_verbose, 
        is_zero_mean_normalization = False
        ):
        ...
        lookup_frames = torch.stack(lookup_frames, 1)  # batch x V_frames x 3 x h x w
        if is_zero_mean_normalization:
            current_image = 2*current_image - 1.0 # to [-1, 1]
            lookup_frames = 2*lookup_frames - 1.0 
        proj_matrices = {}
        gt_masks = {}
        gt_depths = {}
